...
Page({
  ...
  moreListData: function (e) {
    ...
    page++
    util.GET(app.globalData.host + '/Forg/listSuggest', {
      session: wx.getStorageSync('session'),
      page: page,
      pageSize: pageSize
    }, function (res) {
  ...
  getReplayContent:function(e){
    var that = this
    let idx = e.currentTarget.dataset.idx
    var old = that.data.list
    old[idx]['replayContentTmp'] = e.detail.value
    that.setData({list : old})
  },
  replay:function(e){
    var that = this
    ...
    util.GET(app.globalData.host + '/Forg/replaySuggest', {
      session: wx.getStorageSync('session'),
      ...
      if (res && res.code == 1 ) {
        ...
        let idx = e.currentTarget.dataset.idx
        var old = that.data.list
        old[idx]['replayContentTmp'] = ''
        that.setData({ list: old }) 
        that.list()
      } else {
        util.showToast('回复失败', 'error')
      } 
    })
  },
  /**
